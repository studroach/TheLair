...
public class Level1State extends BasicGameState {
	...
	public void enter(GameContainer container, StateBasedGame game) {
		...
		lg = (LairGame)game;
		bat1 = new Monster(432, 288);
	}

	@Override
	public void render(GameContainer container, StateBasedGame game, Graphics g) throws SlickException {
		lg.level.drawRoom(1, g);
		lg.level.drawObjects(1, g);
		//shadows
		lg.level.darken(g);
	...
	public void update(GameContainer container, StateBasedGame game, int delta) throws SlickException {
		...
		if(lg.player.yTile != 0) {
			if((lg.level.lvl1[lg.player.yTile - 1][lg.player.xTile] > 2 && lg.level.lvl1[lg.player.yTile - 1][lg.player.xTile] < 6)
					|| lg.level.lvl1[lg.player.yTile - 1][lg.player.xTile] == 1 || lg.level.lvl1[lg.player.yTile - 1][lg.player.xTile] == 0) {
		...
		if(lg.player.xTile != 0) {
			if((lg.level.lvl1[lg.player.yTile][lg.player.xTile - 1] > 2 && lg.level.lvl1[lg.player.yTile][lg.player.xTile - 1] < 6)
					|| lg.level.lvl1[lg.player.yTile][lg.player.xTile - 1] == 1 || lg.level.lvl1[lg.player.yTile][lg.player.xTile - 1] == 0) {
		...
		if(lg.player.xTile == 15 && lg.player.yTile == 2 && lg.player.getX() < 744) {
			D = true;
		}
		...
		for(int i = 0; i < 6; i++) {
			...
			if((lg.level.level1Objects[i].x > lg.player.xTile - 2 && lg.level.level1Objects[i].x < lg.player.xTile + 2)
					&& (lg.level.level1Objects[i].y > lg.player.yTile - 2 && lg.level.level1Objects[i].y < lg.player.yTile + 2)) {
				
				if(lg.level.level1Objects[i].type == 1 && lg.level.level1Objects[i].state != 0) {
					//door V
					interact = true;
					//this makes doors "solid"
					if(lg.player.getX() > (lg.level.level1Objects[i].x * 48) && lg.player.getX() < (lg.level.level1Objects[i].x * 48) + 48) {
						if(lg.player.getX() < (lg.level.level1Objects[i].x * 48) + 24) {
							D = false;
						}else if(lg.player.getX() > (lg.level.level1Objects[i].x * 48) + 24) {
							A = false;
						}
					}
					//this makes doors open
					if(input.isKeyDown(Input.KEY_E)) {
						if(lg.level.level1Objects[i].state == 1) {
							lg.level.level1Objects[i].state = 0;
						}else if(lg.level.level1Objects[i].state == 2) {
							boolean unlock = false;
							for(int j = 0; j < 6; j++) {
								if(lg.player.keys[j] == lg.level.level1Objects[i].link) {
									unlock = true;
									break;
								}
							}
							if(unlock) {
								lg.level.level1Objects[i].state = 1;
							}else {
								locked = true;
							}
						}
				...
				}else if(lg.level.level1Objects[i].type == 2 && lg.level.level1Objects[i].state != 0) {
					//door H
					interact = true;
					//this makes doors "solid"
					if(lg.player.getY() > (lg.level.level1Objects[i].y * 48) && lg.player.getY() < (lg.level.level1Objects[i].y * 48) + 48) {
						if(lg.player.getY() < (lg.level.level1Objects[i].y * 48) + 24) {
							S = false;
						}else if(lg.player.getY() > (lg.level.level1Objects[i].y * 48) + 24) {
							W = false;
						}
					}
					//this makes doors open
				...
				}else if(lg.level.level1Objects[i].type == 3) {
					//chest
					if(lg.level.level1Objects[i].state == 1) {
						if(input.isKeyDown(Input.KEY_E)) {
							lg.level.level1Objects[i].state = 0;
							lg.player.keys[lg.player.keySlot] = lg.level.level1Objects[i].link;
							lg.player.keySlot++;
						}
						interact = true;
					}
					
				}else if(lg.level.level1Objects[i].type == 4) {
					//lever
					interact = true;
					if(lg.level.level1Objects[i].state == 1) {
						if(input.isKeyDown(Input.KEY_E) && leverCountdown == 0) {
							lg.level.level1Objects[i].lever(lg, 1);
							lg.level.level1Objects[i].state = 0;
							leverCountdown = 500;
						}
					}else if(lg.level.level1Objects[i].state == 0) {
						if(input.isKeyDown(Input.KEY_E) && leverCountdown == 0) {
							lg.level.level1Objects[i].lever(lg, 0);
							lg.level.level1Objects[i].state = 1;
							leverCountdown = 500;
						}
					}
		...
		//pathing
		lg.level.resetPath();
		lg.level.fillPathing(lg.player.xTile, lg.player.yTile, 1, true);
		...
		if(lg.player.xTile == 15 && lg.player.yTile == 2 && lg.player.getX() > 744) {
			lg.player.resetKeys();
			lg.player.setPosition(0,120);
			game.enterState(LairGame.LEVEL2STATE);
		}
